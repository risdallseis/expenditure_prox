...
def fill_empty_lists(df):
    ...
    """
    df['no_reviews'] = df['no_reviews'].apply(lambda y: None if len(y)==1 else y).fillna(0)
    df['recommendation_percent'] = df['recommendation_percent'].apply(lambda y: None if len(y)==1 else y).fillna(0)
    df['review_date'] = df['review_date'].apply(lambda y: None if len(y)==1 else y).fillna(0)
    df['review_rating'] = df['review_rating'].apply(lambda y: None if len(y)==1 else y).fillna(0)
    df['review_text'] = df['review_text'].apply(lambda y: None if len(y)==1 else y).fillna(0)
    df['review_title'] = df['review_title'].apply(lambda y: None if len(y)==1 else y).fillna(0)
    df['summary_star_rating'] = df['summary_star_rating'].apply(lambda y: None if len(y)==1 else y).fillna(0)
